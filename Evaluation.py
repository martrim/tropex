...
def evaluate_x_test(arg, layer_idx):
    ...
    grouped_data, true_labels, network_labels = get_grouped_data(arg, network, max_data_group_size=max_data_group_size,
                                                                 get_labels=True)
    if layer_idx == 'all':
        lower_idx = 0
...
def evaluate_x_train(arg, layer_idx):
    def compute_max_error(layer_idx):
        function_path = get_tropical_function_directory(arg, network, layer_idx, 'training', epoch_number)
        ...
        pos_terms, neg_terms = load_tropical_function(arg, network, 'training', epoch_number, load_negative=True)
        pos_result, neg_result = evaluate_tropical_function(current_data, pos_terms, neg_terms)
...
for epoch_number in epoch_numbers:
    ...
    no_batches = get_no_batches(arg, network)

    if arg.data_type == 'training':
        evaluate_x_train(arg, layer_idx=last_layer_index)
    elif arg.data_type == 'test':
        evaluate_x_test(arg, layer_idx=last_layer_index)
